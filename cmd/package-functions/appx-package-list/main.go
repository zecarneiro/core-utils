...
func setupCommand() {
	cobralib.CobraCmd = &cobra.Command{
		Use:   "appx-package-list",
		Short: "Get list of all Appx Packages or by filter",
	}
...
func process() {
	cmdInfo := models.Command{Cmd: `sudo powershell.exe -Command "Get-AppxPackage -AllUsers | Select-Object Name, PackageFullName, Version | Format-Table"`}
	if len(filter) == 0 {
